<template>
  <div class="main-container">
    ...
    <div class="current-data">
      <div class="current-data__title">{{ labels.otherTypes[$route.params.otherType] }}, Enter</div>
  ...
  methods: {
    ...
    async onScan(decodedText) {
      try {
        ...
        const otherType = this.$route.params.otherType;

        const qrData = QrValitadation.prepareQrData(decodedText, otherType);
        if (!qrData.id) {
        ...
        this.isLoading = true;
        const res = await axios.put(apiBase + `/enter/${otherType}/${qrData.id}`, {
